...
class ViewController: UIViewController {
  ...
  private var argumentsArray: NSArray!
  ...
  override func viewDidLoad() {
    ...
    arguments = OpenCVWrapper.getArguments();

      let data = arguments.data(using: String.Encoding.utf8, allowLossyConversion: true)

      argumentsArray = try! JSONSerialization.jsonObject(with: data!,
    ...
    let featureName = self.modelDataHandler?.recognizeImage(result: result!) ?? ""
      ...
      for argument in argumentsArray {
          if(((argument as  AnyObject)["part_no"] as! String) == featureName){
              brandName = (argument as  AnyObject)["brand"] as! String
              break;
          }
      }
...
//        self.showImageView.image = self.modelDataHandler?.recognizeImage(result: self.result!)
   ...
   */
    func drawAfterPerformingCalculations(onInferences inferences: [Inference], withImageSize imageSize:CGSize,labelName:String?) {
    ...
    for inference in inferences {
      ...
      let confidenceValue = Int(inference.confidence * 100.0)
        let string = "\(labelName ?? "")  (\(confidenceValue)%)"
